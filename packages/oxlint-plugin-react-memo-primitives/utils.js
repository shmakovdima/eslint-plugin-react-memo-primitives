...
// primitive-or-not. Unions/intersections are primitive only if every member is. A
// `TSTypeReference` (`LocaleType`, `MutableRefObject<T>`) is resolved against local
// declarations in the same file (object-shaped ones are handled by
...
//     would be misclassified), but it fixes the common real-world case (enums imported from a
//     shared types file) without reintroducing the original bug (ref/handler objects wrongly
...
//   - `{ kind: "primitive-alias", typeNode }` for `type Foo = <non-object type>` — the caller
//     recurses into typeNode.
...
// known shape at all), this one has a well-known shape — take T's members and add a synthetic
// non-primitive `children` member, since ReactNode is never primitive. T can be either an inline
// object literal or a reference to a locally-declared object-shaped type/interface (resolved via
// resolveLocalTypeDeclaration, same as any other named type reference). Returns null if the
// reference isn't PropsWithChildren, or T isn't an inline object literal or a resolvable local
// object-shaped type.
